...
- Worn weight excludes the **full** `weight × qty`, the same as consumable weight — every worn unit is carried on the body, not in the pack, so it leaves Base Weight entirely. A Placement with `qty: 2, worn: true` on a 500g item contributes 1000g to Total Weight and 1000g to worn weight, leaving 0g in Base Weight.
- The distribution doughnut chart (both `list-summary.vue` and `public-list.vue`) shows **total weight per Category** (`subtotalWeight`), worn and consumable included — a worn Category's slice is its full weight, never its base weight. The worn/consumable exclusion happens only in the footer breakdown rows (`Total` / `Worn` / `Base Weight`), never in the chart slices.
